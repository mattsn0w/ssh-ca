...
#
# Usage: sign.sh relative/path-to-public_key.pub <userid>
...
#OPTIONS="-O no-port-forwarding -O no-pty -O no-x11-forwarding -O permit-agent-forwarding -O no-user-rc -O force-command=/usr/bin/some-script-to-prevent-a-shell.sh"
